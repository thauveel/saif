            ...
            <ul role="list" class="divide-y divide-gray-200">
                ...
                <li class="relative flex justify-between gap-x-6 py-2">
                    ...
                    <span class="relative inline-block">
                        <img class="h-16 w-16 rounded-full" src="{{ asset('storage/' . $tournament->logo) }}" alt="">
                    ...
                    <div class="min-w-0 flex-auto ml-4">
                        ...
                        <a href="mailto:{{$tournament->email}}" class="relative truncate hover:underline">{{$tournament->email}} | {{$tournament->phone}} | {{$tournament->division}}</a>
                        </p>
                    </div>
                    </div>
                    <div class="hidden md:flex items-center gap-x-4">
                        <div class="flex -space-x-0.5">
                            @foreach($tournament->latestteams as $team)
                                <dd>
                                    @if($team->logo)
                                    <img class="h-6 w-6 rounded-full bg-gray-50 ring-2 ring-white" src="{{ asset('storage/' . $team->photo) }}">
                                    @else
